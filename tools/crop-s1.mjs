...
// Series 1 re-crop — same behavior as tools/crop.mjs but sourced from the
// cleaner re-scan (tools/collector-guide-s1.jpg) and its layout-s1.js map.
...
if (FACES.length === 0) {
  console.error('layout-s1.js FACES is empty — calibrate coordinates first.');
  process.exit(1);
}
...
console.log(`Cropped ${count} Series 1 faces -> ${OUT_DIR}`);
