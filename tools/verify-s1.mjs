...
import { SRC, BOX, FACES } from './layout-s1.js';

// Two verification artifacts for Series 3 crop calibration:
//   tools/_overlay-s1.png  — every crop box + name drawn on the half-scale full scan
//   tools/_contact-s1.png  — a labeled contact sheet of the actual crops
// Optional zoom arg for the contact sheet only. Run, view both, adjust layout-s1.js.
...
await sharp({ create: { width: CW, height: CH, channels: 3, background: '#222' } })
  .composite(comp).png().toFile('tools/_contact-s1.png');

console.log(`Wrote tools/_overlay-s1.png (${OW}x${OH}) and tools/_contact-s1.png (${FACES.length} faces)`);
